...
class ConfirmInputController extends Base {
    ...
    const DEFAUL_EMAIL_CONFIRM_TEMPLATE = 'Для подтверждения почтового адреса введите код: {code}';
...
//        }
        ...
        if (in_array("$org_id/$form_id",$this->config->get("armLog", []))&&!empty($_REQUEST['voitingId'])) {
            $this->logArmId = $_REQUEST['voitingId']; //получили разрешение слать лог в АРМ
        }
    ...
    public function sendCode($sso, $value, $type)
    {
        ...
        switch ($type) {
            ...
            case 'email':
                $formTemplate = $this->form['EMAIL_CONFIRM_TEMPLATE'] ?? null;
                $template = $formTemplate ? $formTemplate : self::DEFAUL_EMAIL_CONFIRM_TEMPLATE;
                $body = str_replace(['{code}'], [$code], $template);

                if (!empty($body)) {
                    $mailConf = PoolConfig::me()->get('ispk');
                    $fromEmail = $mailConf->get('from_address');
                    $fromName = $mailConf->get('from_name');
                    $result = (new Ispk($mailConf))->sendEmail(
                        $this->user["PGU_USER_ID"], "Подтверждение почтового адреса", [
                        'email' => $value,
    ...
    public function checkCode($sso, $value, $code, $type, $updateValue = true)
    {
        ...
        if (!empty($oldCode)) {
            ...
            if ($code != $oldCode['code']) {
                $oldCode['attempt'] += 1;
                $formLimit = $this->form['CONFIRM_ATTEMPTS_LIMIT'] ?? null;
                $limit = $formLimit ? $formLimit : self::DEFAUL_CONFIRM_ATTEMPTS_LIMIT;
