 ...
 * Copyright (c) 2013 Jörn Zaefferer; Licensed MIT */(function (t) {
    t.extend(t.fn, {validate: function (e) {
        ...
        //хак для отображения ошибок в случае мульти валидации, и нельзя валид пропускать, если есть ошибки
        return this.checkForm(), t.extend(this.submitted, this.errorMap), this.invalid = t.extend({}, this.errorMap), $.isEmptyObject(this.errorMap)&&this.valid() || t(this.currentForm).triggerHandler("invalid-form", [this]), this.showErrors(this.errorMap), $.isEmptyObject(this.errorMap)&&this.valid()
    }, checkForm: function () {
        this.prepareForm();
        //какая то беда с проверкой формы в целом, так как есть поля, которые дергают свой valid
        var error_arr = {};
        for (var t = 0, e = this.currentElements = this.elements(); e[t]; t++) {
            var d = this.check(e[t]);
            if (!d&&d!=undefined) error_arr[e[t].name] = this.submitted[e[t].name];
        }
        //домешиваем в массив обратно
        $.extend(this.errorMap,error_arr);
        for (var i in error_arr) delete(this.submitted[i]);
        return $.isEmptyObject(this.errorMap)&&this.valid()
    }, element: function (e) {
        ...
        if (t(e).attr("type")=='file'&&($('#helper_'+t(e).attr('id')+'_file_name').attr('id')=='helper_'+t(e).attr('id')+'_file_name')) {s = $('#helper_'+t(e).attr('id')+'_file_name').val(); if (s==undefined) s='';}
        return"radio" === i || "checkbox" === i ? t("input[name='" + t(e).attr("name") + "']:checked").val() : "string" == typeof s ? s.replace(/\r/g, "") : s
    }, check: function (e) {
        //fix для полей без имени
        if ($(e).attr('name')===undefined) return true;
        ...
        //fix для покрашивания valid внутри valid, заменил validElements на successList
        this.successList =	$.extend( this.successList,this.validElements());
        if (this.settings.unhighlight)
        //fix для покрашивания valid внутри valid, заменил validElements на successList
            for (t = 0, e = this.successList; e[t]; t++)
