...
class TaskManager
{
    ...
    public function log($message = '', array $data)
    {
        ...
        if (!self::$result) {
            ...
            self::getLogger()->error($message, array_merge(self::$logData, $data));
        } else {
            if (!empty($message)) {
            ...
            self::getLogger()->info($message, array_merge(self::$logData, $data));
        }
    }
...
//echo 'storeDataInBuffer: ', $userID, ' ', $pluginName, ' ', print_r($data, true), ' ', print_r($params, true), "\n";
    ...
    private function execute($bufferID, $attemptNo, $userID, $data, $pluginName, $params)
    {
        ...
        try {
            ...
            $data = ['class' => get_class($e), 'trace' => $e->getTraceAsString(), 'type' => 'email_send_failure'];
            self::log('PLUGIN ERROR: ' . "{$e->getMessage()}", $data);
        }
    }
