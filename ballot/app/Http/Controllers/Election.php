...
class Election extends Base {
    ...
    public function index($guid, Illuminate\Http\Request $request) {
        try {
            if ($guid && $guid === $this->_config->get('testBallot')) {
                ...
                app()['session.store']->put('district', $userData['okrug']);
            ...
            if ($endTimeStamp-$startTimeStamp<86400) {
                ...
                $timeInterval="с ".date('d',$startTimeStamp)." ".($ru_month[date('m',$startTimeStamp)])." по ".date('d',$endTimeStamp)." ".($ru_month[date('m',$endTimeStamp)]).' '.date('Y',$endTimeStamp).' года';
            }
            ...
            $this->_addTemplateVar('dit_voting', json_encode([
                ...
                'minChoices' => \params::cfgArr($this->_settings, 'min_choices') ?? 1,
                'maxChoices' => \params::cfgArr($this->_settings, 'max_choices') ?? 1,
    ...
    private function _processLogicException(Exceptions\LogicException $e) {
        ...
        $data['errorRaw'] = $e->getMessage();
        $data['errorSystem'] = $e->systemMessage;
        $data['type'] = 'ballot_controller_error';

        $this->_logger->error("{$e->systemMessage}", $data);
        $this->_addTemplateVar('error_message', $message);
        return $this->_renderTemplate('error');
    }
    ...
    private function _processBallotException(Exceptions\BallotException $e, $message = 'Неизвестная ошибка') {
        ...
        $this->_logger->error("{$message}", ['errorRaw' => $e->getMessage(),'type' => 'ballot_controller_error']);
        $this->_addTemplateVar('error_message', $message);
        return $this->_renderTemplate('error');
    }
}