...
class Mgik
{
    ...
    public function __construct(Logging\BaseLogger $logger)
    {
        ...
        $this->_mdmConfig = PoolConfig::me()->get('Mdm');
    }
     ...
     * @throws VotingHasNotStarted
     */
    public function checkBallot($sudir_id, $testMode = false)
    {
        $url = $this->_mdmConfig->get('url/check');
        $token = $this->_mdmConfig->get('token');
        ...
        if ($testMode) {
            ...
            $content['district'] = ['districtNumber' => $_REQUEST['district'] ?? $this->_mdmConfig->get('default_district_id')];
        ...
        $this->_validateAvailabilityForDistrict($district);
    ...
    {
        $url = $this->_mdmConfig->get('url/get');
        $token = $this->_mdmConfig->get('token');
        ...
        if ($testMode) {
            ...
            $content['district'] = ['districtNumber' => $_GET['district'] ?? $this->_mdmConfig->get('default_district_id')];
            $content['random'] = $content['random'] ?? 'random';
            $content['secureHash'] = $content['secureHash'] ?? 'secureHash';
            $content['timestamp'] = $content['timestamp'] ?? time();
        }
    ...
    private function getMpguHashGroup()
    {
        ...
        $data['hash'] = hash('sha256', $data['timestamp'].'|'.$data['random'].'|'.$secret);
     ...
     * @throws VotingHasNotStarted
     */
    private function _validateAvailabilityForDistrict($district): void {
        $voteStartDate = $this->_mdmConfig->get("districts/{$district}/voteStartDate");
        $moscowTimeZone = new \DateTimeZone('+0300');
        $voteStartDateTime = new \DateTime($voteStartDate, $moscowTimeZone);
        if ($voteStartDate !== null && $voteStartDateTime > new \DateTime()) {
            throw new VotingHasNotStarted($voteStartDateTime, $district);
        }
    }
}