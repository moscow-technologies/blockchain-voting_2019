...
class Election
{
    ...
    public function __construct(Ballot $ballotService)
    {
    ...
    public function checkGuid($guid)
    {
        ...
        if (!$district) {
            throw new LogicException('Бюллетень не привязана к избирательному округу.');
        }
        if (!$url) {
            throw new LogicException('Не удается проверить действительность ссылки на бюллетень.');
        }
        if ($opened && !empty($sessId) && $sessId !== $this->_session->getId()) {
            throw new LogicException('Вы уже открывали бюллетень в другом браузере, вы можете отправить голос только на той вкладке браузера');
        }
